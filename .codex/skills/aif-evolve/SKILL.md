...
- Does `$2` mention the most common error categories from patches? If not → add them
- Does `$2` warn about the pitfalls found in patches? If not → add guards
- Does `$2` include logging/validation requirements for problem areas? If not → enhance
- Does `$2` check for the patterns that caused bugs? If not → add checklist items
...
1. **Run `$2`** on recent code to verify improvements
2. **Next evolution** — run `$2` again after 5-10 more fixes
...
|-------|-------------|---------------------|
| `$2` | Patches → common errors | "Check for X before accessing Y" |
| `$2` | Patches → prevention rules | "When creating models, always validate Z" |
| `$2` | Patches → logging gaps | "Add validation task for nullable fields" |
| `$2` | Patches → missed issues | "Check: are all optional relations null-safe?" |
| `$2` | Codebase → conventions | "Use project's commit prefix format" |
...
→ Improvement: Add to $2 checklist:
...
- Do not create new skills (suggest using `$2` instead)
