...
const logger = createLogger('GeminiAiService');
...
export class GeminiAiService implements IAiService {
  ...
  constructor(apiKey: string) {
    ...
    logger.info('GeminiAiService initialized', { textModel: TEXT_MODEL, imageModel: IMAGE_MODEL });
  }
  ...
  /**
   * Call text model (gemini-3-flash-preview) with optional config.
  ...
  private async callTextModel(
    ...
    logger.debug('callTextModel start', { promptLength: prompt.length, hasConfig: !!config });
    ...
    logger.debug('callTextModel done', { elapsed, responseLength: text?.length ?? 0 });
    return text;
  }

  /**
   * Safely parse JSON from an AI response string.
   * Returns parsed value or fallback on failure.
   */
  private parseJsonResponse<T>(raw: string | null, fallback: T): T {
    if (!raw) {
      logger.warn('parseJsonResponse: empty input, returning fallback');
      return fallback;
    }
    try {
      return JSON.parse(raw) as T;
    } catch (err) {
      logger.error('parseJsonResponse: JSON.parse failed', { error: (err as Error).message, rawSnippet: raw.slice(0, 200) });
      return fallback;
    }
  }

  /**
   * Select prompt branch based on niche.
   */
  private buildNichePrompt(
    psychologyPrompt: string,
    ambientPrompt: string,
    niche: Niche,
  ): string {
    return niche === 'psychology' ? psychologyPrompt : ambientPrompt;
  }

  // ---------------------------------------------------------------------------
  // IAiService — JSON-schema methods (Task 20)
  // ---------------------------------------------------------------------------

  async generateIdeas(topic: string, niche: Niche): Promise<VideoIdea[]> {
    logger.debug('generateIdeas called', { topic, niche });

    const prompt = this.buildNichePrompt(
...
ВАЖНО: Канал для Tier-1! Сами идеи (title, hook, primaryKeyword, secondaryKeywords) пиши на АНГЛИЙСКОМ ЯЗЫКЕ. Объяснения (whyItWorks, targetAudience) пиши на РУССКОМ ЯЗЫКЕ для автора.`,
    ...
    const raw = await this.callTextModel(prompt, {
      ...
      responseSchema: {
        ...
        items: {
          type: Type.OBJECT,
          properties: {
            title: { type: Type.STRING, description: 'A highly clickable, intriguing YouTube video title.' },
            hook: { type: Type.STRING, description: 'The first 5-10 seconds of the video script to grab attention.' },
            targetAudience: { type: Type.STRING, description: "Who is this video for? (e.g., 'People struggling with focus')" },
            whyItWorks: { type: Type.STRING, description: 'Psychological reason why viewers will click and watch this.' },
            searchVolume: { type: Type.STRING, description: 'Estimated search interest (High, Medium, or Rising Trend).' },
            primaryKeyword: { type: Type.STRING, description: 'The main SEO search term to target.' },
            secondaryKeywords: { type: Type.ARRAY, items: { type: Type.STRING }, description: '3-5 related long-tail keywords.' },
          },
    ...
    const parsed = this.parseJsonResponse<Omit<VideoIdea, 'niche' | 'createdAt'>[]>(raw, []);
    const ideas: VideoIdea[] = parsed.map((item) => ({
      ...item,
      niche,
      createdAt: new Date(),
    }));

    logger.debug('generateIdeas result', { count: ideas.length });
    return ideas;
  }

  async generateTitles(titleIdea: string): Promise<string[]> {
    logger.debug('generateTitles called', { titleIdea });

    const prompt = `You are an expert YouTube strategist. The user has a video title idea: "${titleIdea}".
...
Return ONLY a JSON array of 5 strings.`;
    ...
    const titles = this.parseJsonResponse<string[]>(raw, []);
    logger.debug('generateTitles result', { count: titles.length });
    return titles;
  }

  async generateBranding(videoTitle: string, niche: Niche): Promise<ChannelBranding | null> {
    logger.debug('generateBranding called', { videoTitle, niche });

    const prompt = `You are an expert YouTube brand strategist. The user has a video idea: "${videoTitle}" in the ${niche} niche.
...
4. A highly detailed image generation prompt for a channel Banner (Cover Art) (на английском).`;
    ...
    const branding = this.parseJsonResponse<ChannelBranding | null>(raw, null);
    logger.debug('generateBranding result', { hasResult: !!branding });
    return branding;
  }

  // ---------------------------------------------------------------------------
  // IAiService — Text generation methods (Task 21)
  // ---------------------------------------------------------------------------

  async generatePlan(title: string, hook: string, niche: Niche): Promise<string> {
    logger.debug('generatePlan called', { title, hook, niche });

    const outlineInstructions = this.buildNichePrompt(
...
- **Video Assembly Workflow (Audio to MP4):** (Provide a step-by-step workflow for turning the final audio track into a YouTube-ready MP4. Explicitly recommend using tools like **Flowjin** for dynamic waveforms/subtitles or **Audiogram** for simple MP3-to-MP4 conversion with visual templates. Explain *how* to use them for this specific video concept.)`,
    ...
    const thumbnailInstructions = this.buildNichePrompt(
...
3. A UI element or icon to add in post-production (e.g., a subtle glowing play button) to instantly signal to the viewer that this is an audio/music experience.)`,
    ...
    const nicheLabel = niche === 'psychology' ? 'psychology/neuroscience' : 'faceless meditation/ambient';

    const prompt = `You are an expert YouTube producer, scriptwriter, and SEO specialist for a ${nicheLabel} channel.
...
(List 2-3 moments where the creator should change visuals, ask a question, or use a sound effect to keep attention. For ambient videos, this could be subtle lighting shifts or new audio layers introduced every hour.)`;
    ...
    if (!text) {
      logger.error('generatePlan returned empty response', { title, niche });
      return 'Failed to generate plan.';
    }
    logger.info('generatePlan success', { title, niche, responseLength: text.length });
    return text;
  }

  async generateDescription(videoTitle: string, planMarkdown: string, niche: Niche): Promise<string | null> {
    logger.debug('generateDescription called', { videoTitle, niche, planLength: planMarkdown.length });

    const prompt = `You are an expert YouTube SEO specialist and copywriter.
...
Format as plain text or minimal Markdown.`;
    ...
    const text = await this.callTextModel(prompt);
    if (!text) logger.error('generateDescription returned empty', { videoTitle, niche });
    else logger.info('generateDescription success', { videoTitle, responseLength: text.length });
    return text;
  }

  async generateNotebookLMSource(videoTitle: string, planMarkdown: string, niche: Niche): Promise<string | null> {
    logger.debug('generateNotebookLMSource called', { videoTitle, niche, planLength: planMarkdown.length });

    const prompt = `You are an expert content strategist and podcast producer.
...
Format this as a clean, readable text document. Do not use markdown formatting that might confuse a text-to-speech engine (keep it conversational and structured).`;
    ...
    const text = await this.callTextModel(prompt);
    if (!text) logger.error('generateNotebookLMSource returned empty', { videoTitle, niche });
    else logger.info('generateNotebookLMSource success', { videoTitle, responseLength: text.length });
    return text;
  }

  async generateShortsSpinoffs(videoTitle: string, planMarkdown: string): Promise<string | null> {
    logger.debug('generateShortsSpinoffs called', { videoTitle, planLength: planMarkdown.length });
...
Format as Markdown.`;
    ...
    const text = await this.callTextModel(prompt);
    if (!text) logger.error('generateShortsSpinoffs returned empty', { videoTitle });
    else logger.info('generateShortsSpinoffs success', { videoTitle, responseLength: text.length });
    return text;
  }

  async generateMonetizationCopy(videoTitle: string, niche: Niche): Promise<string | null> {
    logger.debug('generateMonetizationCopy called', { videoTitle, niche });

    const prompt = `You are an expert copywriter for creators.
The user is creating a YouTube video titled: "${videoTitle}" in the ${niche} niche.
...
Format as Markdown.`;
    ...
    const text = await this.callTextModel(prompt);
    if (!text) logger.error('generateMonetizationCopy returned empty', { videoTitle, niche });
    else logger.info('generateMonetizationCopy success', { videoTitle, responseLength: text.length });
    return text;
  }

  async generateContentRoadmap(videoTitle: string, niche: Niche): Promise<string | null> {
    logger.debug('generateContentRoadmap called', { videoTitle, niche });

    const prompt = `You are a YouTube Channel Strategist.
...
Format as Markdown.`;
    ...
    const text = await this.callTextModel(prompt);
    if (!text) logger.error('generateContentRoadmap returned empty', { videoTitle, niche });
    else logger.info('generateContentRoadmap success', { videoTitle, responseLength: text.length });
    return text;
  }

  async generateSunoPrompt(videoTitle: string, planMarkdown: string): Promise<string | null> {
    logger.debug('generateSunoPrompt called', { videoTitle, planLength: planMarkdown.length });

    const prompt = `You are an expert audio engineer and AI music prompt specialist.
...
Return ONLY the prompt string, nothing else.`;
    ...
    const text = await this.callTextModel(prompt);
    if (!text) logger.error('generateSunoPrompt returned empty', { videoTitle });
    else logger.info('generateSunoPrompt success', { videoTitle, responseLength: text.length });
    return text;
  }

  // ---------------------------------------------------------------------------
  // IAiService — Special methods (Task 22)
  // ---------------------------------------------------------------------------

  async generateThumbnail(prompt: string): Promise<string | null> {
    const startMs = Date.now();
    logger.debug('generateThumbnail called', { promptLength: prompt.length });
    ...
    for (const part of parts) {
      if (part.inlineData) {
        ...
        const elapsed = Date.now() - startMs;
        logger.info('generateThumbnail success', { elapsed, mimeType: part.inlineData.mimeType, dataLength: part.inlineData.data?.length });
        return dataUrl;
      }
    }

    logger.warn('generateThumbnail: no image data in response', { candidateCount: response.candidates?.length });
    return null;
  }

  async analyzeNiche(videoTitle: string, niche: Niche): Promise<string | null> {
    const startMs = Date.now();
    logger.debug('analyzeNiche called (Google Search grounding)', { videoTitle, niche });
...
Format as Markdown.`;
    ...
    if (!text) logger.error('analyzeNiche returned empty', { videoTitle, niche, elapsed });
    else logger.info('analyzeNiche success (search-grounded)', { videoTitle, elapsed, responseLength: text.length });
    return text;
  }
}
